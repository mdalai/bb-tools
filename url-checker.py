...
class urlChecker(object):
    ...
    def workURL(self, htmlText):
        page =htmlText.encode("utf-8")
        start_pos = 0
        while True:
            url,endpos = get_next_url(self,htmlText,start_pos)
            if url:

                self.new_html = self.new_html + page[:start_pos]
                start_pos = endpos
    ...
    def get_next_url(self,page,s_pos):
        a_link = page.find('<a ', s_pos)
...
class htmlEdit(object):
    ...
    def youtubeEmbeddedMaker(self,htmlText):
        ...
        start_pos = 0
        while htmlText.find('<a href="https://youtu.be',start_pos)!= -1:
            ahref_start_pos = htmlText.find('<a href="https://youtu.be',start_pos)
            ahref_end_pos = htmlText.find('"',ahref_start_pos+12)
            url = htmlText[ahref_start_pos +9: ahref_end_pos]
            # Extracting YOUTUBE_ID from URL
            yid = self.getYoutubeID(url)
